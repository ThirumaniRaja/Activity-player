var McqScqCompCustom_v2 = function() {
  var p = {
    __vRef: 'OFF', // [Inspectable(name="08 Visual Reference",enumeration="OFF, ON - Tick/Cross",defaultValue="OFF")]
    __submitMc: '', // [Inspectable(name="05 SubmitBtn MC Instance",type="String")]
    __solutionMc: '', // [Inspectable(name="06 SolutionBtn MC Instance",type="String")]
    __feedbackMc: '', //[Inspectable(name="07 Feedback MC Instance",type="String")]
    __questionType: 'SingleChoice', //[Inspectable(name="01 Type of question",enumeration="SingleChoice,MultipleChoice",defaultValue="SingleChoice")]
    __customFnCall: '', // [Inspectable(name="10 Custom function",type="String")]
    __gotoLabel: '', // [Inspectable(name="11 GoTo Label",type="String")]
    __radioGroupArr: [], // [Inspectable(name="03 Question MC Instance",type="Array")]
    __correctAnswers: [], // [Inspectable(name="04 Answers MC Instance",type="Array")]
    __maxNumberOfTries: 0, //[Inspectable(name="02 Maximum Attempt",type="int",defaultValue=0)]
    __fbSelFlag: false, //[Inspectable(name = "09 Feedback as per selection", type = "Boolean")]
    __showIndividualAnimation: false, //[Inspectable(name="12 Show Individual Status",type="Boolean")]
    __numberOfCorrectFeedbacks: 1, //	[Inspectable(name="13 Number Of Correct Feedbacks",type="int",defaultValue=1)]
    __numberOfIncorrectFeedbacks: 2, // [Inspectable(name="14 Number Of Incorrect Feedbacks",type="int",defaultValue=2)]
    __resetIncorrectItemsGroup: true, //[Inspectable(name="15 Reset Incorrect(Group)",type="Boolean",defaultValue=true)]
    __questNo: 0, // [Inspectable(name="17 Question Number",type="int",defaultValue=0)]
    __maintainScore: false, //[Inspectable(name = "16 Maintain Score", type = "Boolean")]
  ...
  function resetCanvas(_arr, _notReset) {
    for (var i = 0; i < _arr.length; i++) {
      if (_notReset) {
        ...
        //removed for multiple select on canvas
        /*if(p.resetAllOptionsAfterIncorrectFeedBack){
					if(p.domObj[_arr[i]] && p.domObj[_arr[i]].getAttribute('_status') == 'true'){
						gotoAndStopRadioCanas(p.domObj[_arr[i]+'_option'] , "normal");
						p.domObj[_arr[i]].setAttribute('_status','false');
					}
				}*/
  ...
  function canvasEvents(e) {
    ...
    if (_target) {
      ...
      if (e.type == 'mousemove') {
        if (!multipleChoice()) {
          resetCanvas(
            p.__radioGroupArr[Number(_currentGroup)],
          ...
          resetCanvas(p.__radioGroupArr[Number(_currentGroup)]);
          var _temp = getCurrentCanvasId(
            p.__radioGroupArr[Number(_currentGroup)],
            e.pageX,
            e.pageY
      ...
      if (e.type == 'mousedown' || e.type == 'touchstart') {
        ...
        if (optionSelected) {
          ...
          if (p.__submitMc == '' && !multipleChoice()) {
  ...
  function startActivity() {
    ...
    assignListeners();
    if (p.__feedbackMc != '') {
      fbMC = p.domObj[p.__feedbackMc];
    }
    if (p.__submitMc != '') {
      sbMC = p.domObj[p.__submitMc];
      gotoAndStopSubmit(sbMC, 'disable');
    }
    if (p.__solutionMc != '') {
      solMC = p.domObj[p.__solutionMc];
  ...
  function assignListeners() {
    ...
    tempMc._group = -1;
    for (var i = 0; i < p.__radioGroupArr.length; i++) {
      var tempStr = String(p.__radioGroupArr[i]);
    ...
    if (p.__maintainScore) {
      addstar_mc = new StarController(shellController, p._shellModel);
  ...
  function createTickAndCrossDiv(_elem) {
    ...
    $(_elem).append(_div);
    var _img = 'cross';
    if (p.__correctAnswers.indexOf(_elem.id) != -1) {
      _img = 'tick';
    }
    $(_div)
      .css({
    ...
    var _left = 0 - activityImageObj['tick'].width - Number(p.tickmarginLeft),
      _top = 0 + Number(p.tickmarginTop);
    ...
    if (p.tickAndCrossPosition) {
      ...
      if (p.tickAndCrossPosition['x']) {
        _left = p.tickAndCrossPosition['x'];
      }
      if (p.tickAndCrossPosition['y']) {
        _top = p.tickAndCrossPosition['y'];
      }
    }
    $(_div).css({
      top: _top + 'px',
  ...
  function getAttemptedAns() {
    ...
    for (var i = 0; i < p.__radioGroupArr.length; i++) {
      ...
      for (var j = 0; j < p.__radioGroupArr[i].length; j++) {
        var mc = p.domObj[p.__radioGroupArr[i][j]];
        if (mc) {
  ...
  function checkForValidation() {
    ...
    if (incorrectFeedbackCounter >= p.__numberOfIncorrectFeedbacks) {
      incorrectFeedbackCounter = p.__numberOfIncorrectFeedbacks;
    }
    if (correctFeedbackCounter >= p.__numberOfCorrectFeedbacks) {
      correctFeedbackCounter = p.__numberOfCorrectFeedbacks;
    }
    //  if limited attempts
    if (p.__maxNumberOfTries != 0) {
      counter++;
      if (counter < p.__maxNumberOfTries) {
        validateAnswer();
        //actionHandler();
      } else if (counter == p.__maxNumberOfTries) {
        validateAnswer();
        //actionHandler();
  ...
  function resetRadioOptions() {
    if (currentOption) {
      var currentGroup = parseInt(currentOption.getAttribute('_group'));
      var tempStr = String(p.__radioGroupArr[currentGroup]);
      var tempArr = tempStr.split(',');
  ...
  function showAllCorrectAnswerTick() {
    for (var i = 0; i < p.__correctAnswers.length; i++) {
      var _mc = p.domObj[p.__correctAnswers[i]];
      if (_mc) {
  ...
  function setAllCorrectOptionsSelected() {
    for (var i = 0; i < p.__correctAnswers.length; i++) {
      var _mc = p.domObj[p.__correctAnswers[i] + '_option'];
      if (_mc) {
        if (p.detectTransparency) {
  ...
  function addSubmitListener() {
    if (!multipleChoice()) {
      ...
      if (_count == p.__correctAnswers.length) {
        tempBool = true;
      }
      if (tempBool) {
        // if (submitBool)
      ...
      if (p.__gotoLabel != '') {
        // need to checck what happen because of this
        //__refToStage.gotoAndStop(__gotoLabel);
      }
    }
    if (multipleChoice()) {
  ...
  function validateAnswer() {
    ...
    if (p.__showIndividualAnimation) {
      ...
      for (var i = 0; i < p.__radioGroupArr.length; i++) {
        for (var j = 0; j < p.__radioGroupArr[i].length; j++) {
          var _mc = p.domObj[String(p.__radioGroupArr[i][j])];
          if (_mc) {
            if (_mc.getAttribute('_status') == 'true') {
              attemptedMultipleAnswersMc.push(p.__radioGroupArr[i][j]);
            }
          }
        }
      }
      ...
      for (var i = 0; i < attemptedMultipleAnswersMc.length; i++) {
        if (p.__correctAnswers.indexOf(attemptedMultipleAnswersMc[i]) != -1) {
    ...
    //  corretct answer
    if (String(attemptedMultipleAnswersMc) == String(p.__correctAnswers)) {
      actionHandler('correct');
      gotoAndPlayFeedBack('correct', 'correct' + correctFeedbackCounter);
      //AddScore();
    } else {
      // inCorrect answers
      actionHandler('inCorrect');
      gotoAndPlayFeedBack('inCorrect', 'inCorrect' + incorrectFeedbackCounter);
      //AddScore();
    }
  }
    ...
    if (p.__maintainScore) {
      if (addstar_mc != null) {
        ...
        if (_ans == 'correct') {
          ...
          p._shellModel.setPageData(p.__questNo, { value: 1, glow: true });
          shellController.updateStar();
        } else {
          //addstar_mc.updateAnswer(p.__questNo,0);
          p._shellModel.setPageData(p.__questNo, { value: 0, glow: false });
          shellController.updateStar();
        }
      }
    }
  ...
  function addshowSolListener() {
    if (p.__solutionMc != '') enableShowSolution();
  }
  ...
  function showSolution(e) {
    if (e.type == 'mousedown') {
      ...
      //	resetRadioOptions();
      //}
      var tempcorrectAnswersArr = String(p.__correctAnswers).split(',');
      ...
      setOptionsDisable();
      gotoAndPlayFeedBack('solution', 'solution');
      ...
      //if(p.__resetIncorrectItemsGroup)
      //resetIncorrectItems();
    }
    if (e.type == 'mouseover') {
  ...
  function resetIncorrectItems() {
    if (p.__resetIncorrectItemsGroup) {
      if (!multipleChoice()) {
        for (var k = 0; k < p.__correctAnswers.length; k++) {
          if (attemptedAnswers[k] != p.__correctAnswers[k]) {
      ...
      if (multipleChoice()) {
        for (var i = 0; i < attemptedAnswersMc.length; i++) {
          if (attemptedAnswersMc[i]) {
            for (var j = 0; j < attemptedAnswersMc[i].length; j++) {
              if (p.__correctAnswers.indexOf(attemptedAnswersMc[i][j]) == -1) {
  ...
  function gotoAndPlayFeedBack(_status, _id) {
    if (p.navFbStartJson) {
  ...
  function multipleChoice() {
    if (p.__questionType == 'SingleChoice') {
    ...
    if (!multipleChoice()) {
      for (var i = 0; i < p.__radioGroupArr.length; i++) {
        if (attemptedAnswersMc[i] && attemptedAnswersMc[i].length > 0) {
          if (p.__correctAnswers.indexOf(attemptedAnswersMc[i][0]) == -1) {
            for (var j = 0; j < p.__radioGroupArr[i].length; j++) {
              var _mc = p.domObj[p.__radioGroupArr[i][j]];
              if (_mc) {
                if (p.detectTransparency) {
        ...
        for (var j = 0; j < p.__radioGroupArr[i].length; j++) {
          ...
          if (_mc) {
            if (
              ...
              p.__correctAnswers.indexOf(p.__radioGroupArr[i][j]) != -1
            ) {
              if (p.resetAllOptionsAfterIncorrectFeedBack) {
                if (p.detectTransparency) {
                  gotoAndStopRadioCanas(p.domObj[_mc.id + '_option'], 'normal');
                ...
                attemptedAnswersMc[i].splice(
                  attemptedAnswersMc[i].indexOf(p.__radioGroupArr[i][j]),
                  1
                );
              }
            } else {
  ...
  function callFrmComp() {
    if (p.__customFnCall != '') {
      //var custFunction:Function = __refToStage[customFnCall];
      //var custFunction:Function = __refToStage[customFnCall];
      //custFunction();
    }
  }
