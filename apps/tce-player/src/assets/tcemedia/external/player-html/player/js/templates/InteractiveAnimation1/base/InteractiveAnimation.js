...
var InteractiveAnimation = function() {
  ...
  function gotoAndStopTab(elem, state) {
    ...
    //	//////////////console.log(state,"state",p._btnCssObj[state])
    //////////////console.log(elem,"_imgObj",state)
    if (p._imgObj[elem]) {
      if (p._imgObj[elem][state] && !p._trasparancy) {
  ...
  function bindEvents(elemId) {
    ...
    var a = $(p.domObj[elemId]).attr('merge');
    if (typeof a != 'undefined' && a == 'true') {
      $(p.domObj[elemId]).css({ 'z-index': 0 });
    }
    $(p.domObj[elemId])
  ...
  function unbindEvents(elemId) {
    ...
    if (p.feedBack) {
      p.feedBack({
        videoFb: p.feedbackParam[p._controlMc01],
        elemId: elemId
      });
    }
    $(p.domObj[p._id]).removeClass('addPointer');
    ////////console.log("unbindEvents : " , elemId);
    var a = $(p.domObj[elemId]).attr('merge');
    if (typeof a != 'undefined' && a == 'true') {
      $(p.domObj[elemId]).css({ 'z-index': 1 });
    }
    ...
    if (p.showControlHolder && p.showControlHolder.visible) {
      ...
      if (p.showControlHolder.visibleSmall) {
        ...
        if (p.showControlHolder.smallwidth) {
          $(p.domObj['lowerDiv']).css({
            width: p.showControlHolder.smallwidth
          });
        }
      } else {
        $(p.domObj['upperDiv']).css({
  ...
  function showHeader() {
    if (p.header) {
      ...
      $(p.domObj['bg_header']).html(p._shellModel.getTextValue(p.header.txtId));
      var tempTxt = $(p.domObj['bg_header'])
        .children()
        .eq(0)[0];
