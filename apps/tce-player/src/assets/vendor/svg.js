 ...
 */ (function(root, factory) {
  ...
  SVG.adopt = function(node) {
    ...
    if (node.nodeName == 'svg')
      element =
        node.parentNode instanceof window.SVGElement
          ? new SVG.Nested()
          : new SVG.Doc();
  ...
  SVG.prepare = function() {
    ...
    var body = document.getElementsByTagName('body')[0],
      draw = (body
        ? new SVG.Doc(body)
        : SVG.adopt(document.documentElement).nested()
      ).size(2, 0);
  ...
  SVG.extend(SVG.PointArray, {
    ...
    size: function(width, height) {
      ...
      for (i = this.value.length - 1; i >= 0; i--) {
        if (box.width)
          this.value[i][0] =
            ((this.value[i][0] - box.x) * width) / box.width + box.x;
        if (box.height)
          this.value[i][1] =
            ((this.value[i][1] - box.y) * height) / box.height + box.y;
      }
  ...
  SVG.extend(SVG.PathArray, {
    ...
    size: function(width, height) {
      ...
      for (i = this.value.length - 1; i >= 0; i--) {
        ...
        if (l == 'M' || l == 'L' || l == 'T') {
          this.value[i][1] =
            ((this.value[i][1] - box.x) * width) / box.width + box.x;
          this.value[i][2] =
          ...
          this.value[i][2] =
            ((this.value[i][2] - box.y) * height) / box.height + box.y;
          this.value[i][3] =
            ((this.value[i][3] - box.x) * width) / box.width + box.x;
          this.value[i][4] =
            ((this.value[i][4] - box.y) * height) / box.height + box.y;

          if (l == 'C') {
            this.value[i][5] =
              ((this.value[i][5] - box.x) * width) / box.width + box.x;
            this.value[i][6] =
              ((this.value[i][6] - box.y) * height) / box.height + box.y;
          }
        } else if (l == 'A') {
          // resize radii
          ...
          this.value[i][6] =
            ((this.value[i][6] - box.x) * width) / box.width + box.x;
          this.value[i][7] =
            ((this.value[i][7] - box.y) * height) / box.height + box.y;
        }
      }
    ...
    at: function(pos) {
      ...
      for (i = 0, il = sourceArray.length; i < il; i++) {
        ...
        for (j = 1, jl = sourceArray[i].length; j < jl; j++) {
          array[i][j] =
            sourceArray[i][j] +
            (destinationArray[i][j] - sourceArray[i][j]) * pos;
        }
  ...
  SVG.Element = SVG.invent({
    ...
    extend: {
      ...
      size: function(width, height) {
        ...
        return this.width(new SVG.Number(p.width)).height(
          new SVG.Number(p.height)
        );
      },
      // Clone element
      ...
      parent: function(type) {
        ...
        while (parent && parent.node instanceof window.SVGElement) {
          ...
          if (
            ...
            parent.node.parentNode.nodeName == '#document' ||
            parent.node.parentNode.nodeName == '#document-fragment'
          )
            return null; // #759, #720
          parent = SVG.adopt(parent.node.parentNode);
        }
  ...
  SVG.FX = SVG.invent({
    ...
    extend: {
      ...
      timeToAbsPos: function(timestamp) {
        return (
          (timestamp - this.situation.start) /
      ...
      absPosToTime: function(absPos) {
        return (
          (this.situation.duration / this._speed) * absPos +
      ...
      during: function(fn) {
        ...
        // see above
        this.target()
          .off('during.fx', wrapper)
          .on('during.fx', wrapper);
      ...
      afterAll: function(fn) {
        ...
        // see above
        this.target()
          .off('allfinished.fx', wrapper)
          .on('allfinished.fx', wrapper);
      ...
      step: function(ignoreTime) {
        ...
        if (this.situation.loops !== false) {
          ...
          if (
            this.situation.loops === true ||
            absPosInt < this.situation.loops
          ) {
        ...
        if (s.transforms.length) {
          ...
          for (i = 0, len = s.transforms.length; i < len; i++) {
            ...
            if (a instanceof SVG.Matrix) {
              if (a.relative) {
                at = at.multiply(
                  new SVG.Matrix().morph(a).at(s.ease(this.pos))
                );
              } else {
    ...
    construct: {
      ...
      animate: function(o, ease, delay) {
        return (this.fx || (this.fx = new SVG.FX(this))).animate(
          o,
          ease,
          delay
        );
      ...
      speed: function(speed) {
        if (this.fx)
          if (speed == null) return this.fx.speed();
  ...
  SVG.extend(SVG.FX, {
    ...
    size: function(width, height) {
      if (this.target() instanceof SVG.Text) {
        ...
        this.add('width', new SVG.Number(width)).add(
          'height',
          new SVG.Number(height)
        );
      }
  ...
  SVG.Matrix = SVG.invent({
    ...
    create: function(source) {
      ...
      for (i = abcdef.length - 1; i >= 0; --i)
        this[abcdef[i]] =
          source[abcdef[i]] != null ? source[abcdef[i]] : base[abcdef[i]];
    },
    ...
    extend: {
      ...
      multiply: function(matrix) {
        return new SVG.Matrix(
          this.native().multiply(parseMatrix(matrix).native())
        );
      },
      // Inverses matrix
      inverse: function() {
      ...
      skew: function(x, y, cx, cy) {
        ...
        return this.around(
          cx,
          cy,
          new SVG.Matrix(1, Math.tan(y), Math.tan(x), 1, 0, 0)
        );
      },
      // SkewX
      ...
      around: function(cx, cy, matrix) {
        return this.multiply(new SVG.Matrix(1, 0, 0, 1, cx || 0, cy || 0))
          .multiply(matrix)
      ...
      if (o.a != null) {
        ...
        if (!relative) {
          ...
          matrix = matrix.multiply(
            new SVG.Matrix().skew(e.skewX, e.skewY, o.cx, o.cy).inverse()
          );
        }
  ...
  SVG.Transformation = SVG.invent({
    ...
    extend: {
      ...
      at: function(pos) {
        ...
        m = new SVG.Matrix()
          .morph(SVG.Matrix.prototype[this.method].apply(m, params))
          .at(pos);
      ...
      undo: function(o) {
        for (var i = 0, len = this.arguments.length; i < len; ++i) {
          o[this.arguments[i]] =
            typeof this[this.arguments[i]] == 'undefined'
              ? 0
              : o[this.arguments[i]];
        }
  ...
  SVG.extend(SVG.Element, {
    ...
    style: function(s, v) {
      if (arguments.length == 0) {
        ...
        this.node.style[camelCase(s)] =
          v === null || SVG.regex.isBlank.test(v) ? '' : v;
      }
  ...
  SVG.extend(SVG.Parent, {
    ungroup: function(parent, depth) {
      if (
        depth === 0 ||
        this instanceof SVG.Defs ||
        this.node == SVG.parser.draw
      )
        return this;

      parent =
        parent || (this instanceof SVG.Doc ? this : this.parent(SVG.Parent));
  ...
  SVG.ViewBox = SVG.invent({
    create: function(source) {
      ...
      if (source instanceof SVG.Element) {
        ...
        while (width.unit == '%') {
          wm *= width.value;
          width = new SVG.Number(
            we instanceof SVG.Doc
              ? we.parent().offsetWidth
              : we.parent().width()
          );
          we = we.parent();
        }
        while (height.unit == '%') {
          hm *= height.value;
          height = new SVG.Number(
            he instanceof SVG.Doc
              ? he.parent().offsetHeight
              : he.parent().height()
          );
          he = he.parent();
        }
        ...
        source =
          typeof source === 'string'
            ? source.match(reg).map(function(el) {
  ...
  SVG.G = SVG.invent({
    ...
    extend: {
      ...
      y: function(y) {
        return y == null
          ? this.transform('y')
          : this.transform({ y: y - this.y() }, true);
      },
      // Move by center over x-axis
      cx: function(x) {
  ...
  SVG.Doc = SVG.invent({
    ...
    extend: {
      ...
      spof: function() {
        ...
        if (pos)
          this.style('left', (-pos.e % 1) + 'px').style(
            'top',
            (-pos.f % 1) + 'px'
          );
  ...
  SVG.extend(SVG.Element, {
    ...
    clipWith: function(element) {
      ...
      this.clipper =
        element instanceof SVG.ClipPath
          ? element
          : this.parent()
              .clip()
              .add(element);
  ...
  SVG.extend(SVG.Defs, {
    // Define gradient
    pattern: function(width, height, block) {
      return this.put(new SVG.Pattern())
        .update(block)
        .attr({
          x: 0,
          y: 0,
  ...
  SVG.Circle = SVG.invent({
    ...
    construct: {
      ...
      circle: function(size) {
        return this.put(new SVG.Circle())
          .rx(new SVG.Number(size).divide(2))
          .move(0, 0);
      }
    }
  });
  ...
  SVG.extend(SVG.Circle, SVG.Ellipse, {
    ...
    width: function(width) {
      return width == null
        ? this.rx() * 2
        : this.rx(new SVG.Number(width).divide(2));
    },
    // Set height of element
    height: function(height) {
      return height == null
        ? this.ry() * 2
        : this.ry(new SVG.Number(height).divide(2));
    },
  ...
  SVG.extend(SVG.Polyline, SVG.Polygon, {
    ...
    array: function() {
      return (
        this._array || (this._array = new SVG.PointArray(this.attr('points')))
      );
    },
    // Plot new path
    plot: function(p) {
  ...
  SVG.extend(SVG.Element, SVG.FX, {
    ...
    dx: function(x) {
      return this.x(
        new SVG.Number(x).plus(this instanceof SVG.FX ? 0 : this.x()),
        true
      );
    },
    ...
    dy: function(y) {
      return this.y(
        new SVG.Number(y).plus(this instanceof SVG.FX ? 0 : this.y()),
        true
      );
    },
    ...
    methods.forEach(function(method) {
      SVG.Set.prototype[method] = function() {
        ...
        return method == 'animate'
          ? this.fx || (this.fx = new SVG.FX.Set(this))
          : this;
      };
    });
  ...
  SVG.select = function(query, parent) {
    return new SVG.Set(
      SVG.utils.map((parent || document).querySelectorAll(query), function(
        node
      ) {
        return SVG.adopt(node);
      })
