...
function erase(paths, erasePath, eraseRadius) {
  //console.log("erasejs paths --->> "+paths)
  //console.log("erasejs erasePath --->> "+erasePath)
  //console.log("ersaePaths --->> "+erasePath)
  //console.log("eraseRadius --->> "+eraseRadius)
  //var date = new Date();
  ...
  var pointErase = function(path) {
    ...
    while (i < path.length - 1) {
      ...
      //   contribute to a new path.  Processing then continues at p1.
        ...
        var possIntersects = getCircleIntersections(
          p0[0],
          p0[1],
          p1[0],
          p1[1],
        ...
        if (possIntersects) {
          ...
          if (
            path[i + 1] &&
            path[i + 1][0] == possIntersects[1][0] &&
            path[i + 1][1] == possIntersects[1][1]
          )
            i++;
          last = i;
        } else {
  ...
  var capsuleErase = function(path, i) {
    ...
    // handle point path
    //console.log("capsule path.length --- "+path.length)
    if (path.length === 1) {
      var p0_locationIndex = withinCapsule(
        path[0][0],
        path[0][1],
        e0[0],
        e0[1],
    ...
    while (i < path.length - 1) {
      ...
      var p1_locationIndex = withinCapsule(
        p1[0],
        p1[1],
        e0[0],
        e0[1],
      ...
      //console.log("p1_locationIndex--",p1_locationIndex)
      //console.log("e0--",e0)
      //console.log("e1--",e1)
      // if both points are in the erase area, the first point does not contribute to a new path and can be ignored
      if (
        p0_locationIndex.indexOf(1) !== -1 &&
        p1_locationIndex.indexOf(1) !== -1
      ) {
        i++;
        last = i;
      }
      ...
      //   contribute to a new path.  Processing then continues at p1.
        ...
        // Neither p0 or p1 is in the erase area, so there may or may not be a pair of intersections.
        var possIntersects = getCapsuleIntersections(
          p0[0],
          p0[1],
          p1[0],
          p1[1],
...
function cleanPath(path) {
  ...
  if (path.length === 1) {
    ...
    while (pClean < path.length - 1) {
      if (
        path[pClean][0] !== path[pClean + 1][0] ||
        path[pClean][1] !== path[pClean + 1][1]
      ) {
...
function getCircleIntersection(aX, aY, bX, bY, cX, cY, r) {
  ...
  var rightPoint = [
    aX + ac_proj_ab * u_vec_ab[0],
    aY + ac_proj_ab * u_vec_ab[1]
  ];
  var distCToRightPoint = Math.sqrt(
    Math.pow(cX - rightPoint[0], 2) + Math.pow(cY - rightPoint[1], 2)
  );
  var b;
 ...
 */
function getCapsuleIntersection(
  aX,
  aY,
  locationIndex,
  bX,
  ...
  for (var n = 0; n < intersections.length; n++) {
    if (intersections[n]) {
      if (
        getDistance(intersections[n][0], intersections[n][1], bX, bY) <
...
function getCapsuleIntersections(aX, aY, bX, bY, c0_x, c0_y, c1_x, c1_y, r) {
  ...
  for (var n = 0; n < intersections.length; n++) {
    if (intersections[n]) {
      if (
        getDistance(intersections[n][0], intersections[n][1], aX, aY) <
  ...
  for (var m = 0; m < intersections.length; m++) {
    if (intersections[m]) {
      if (
        getDistance(intersections[m][0], intersections[m][1], bX, bY) <
  ...
  if (
    (intersection0[0] === bX && intersection0[1] === bY) ||
    (intersection1[0] === aX && intersection1[1] === aY)
  )
    return null;
